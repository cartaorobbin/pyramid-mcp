    ...
    if username:
        ...
        if user_data:
            return UserOwnedContext(
                owner_username=username, owner_userid=user_data["id"]
            )
...
def get_profile(request):
    ...
    if user["auth_type"] == "jwt":
        ...
        if user_data:
            return {"user": user_data["profile"], "role": user["role"]}
...
def get_user_profile(request):
    ...
    return {"user": user_data["profile"]}


@view_config(
    route_name="user_profile", renderer="json", request_method="PUT", permission="edit"
)
...
@tool(
    name="system_status",
    description="Get system status (admin context required)",
    permission="view",
)
...
@tool(
    ...
    description="Process data with authenticated context",
    permission="view",
)
...
def create_app(global_config=None, **settings):
    ...
    # Filter out Pyramid-specific settings that shouldn't be passed to Configurator
    filtered_settings = {
        k: v for k, v in settings.items() if not k.startswith("pyramid.")
    }
    ...
    config.add_route(
        "auth_profile", "/auth/profile", factory=authenticated_context_factory
    )
    config.add_route(
        "api_secure_data", "/api/secure-data", factory=authenticated_context_factory
    )
    ...
    config.add_route(
        "user_profile", "/users/{username}", factory=user_profile_context_factory
    )

    # Dynamic context routes
    config.add_route("calculator", "/calculator", factory=calculator_context_factory)

    # Include pyramid_mcp with route discovery enabled
