...
def test_discover_tools_with_patterns(pyramid_config_committed, mcp_config_with_patterns):
...
def test_tool_name_generation_edge_cases():
    ...
    assert introspector._generate_tool_name('api_v1_users', 'GET', '/api/v1/users') == 'list_api_v1_users'
    assert introspector._generate_tool_name('complex_route', 'PATCH', '/complex/{id}') == 'modify_complex_route'
...
def test_input_schema_complex_types():
    ...
    schema = introspector._generate_input_schema('/items/{item_id}', complex_view, 'PUT')
...
def test_pattern_matching_advanced():
    ...
    assert not introspector._pattern_matches('*/api/*', '/v1/auth/login', 'v1_auth_login')
...
def test_description_generation():
    ...
    desc1 = introspector._generate_tool_description('test_route', 'GET', '/test', documented_view)
    assert 'documented' in desc1.lower() or 'test' in desc1.lower()
    
    # Test with undocumented view
    desc2 = introspector._generate_tool_description('other_route', 'POST', '/other', undocumented_view)
