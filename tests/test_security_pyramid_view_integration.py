...
def test_pyramid_view_with_bearer_auth_integration():
    ...
    @view_config(
        route_name="secure_api_endpoint", renderer="json", mcp_security="bearer"
    )
    ...
    list_response = app.post_json(
        "/mcp", {"jsonrpc": "2.0", "method": "tools/list", "id": 2}  # type: ignore
    )
    ...
    secure_tool_name = (
        "get_secure_api_endpoint"  # Auto-generated from route name + method
    )
    secure_tool = next((t for t in tools if secure_tool_name in t["name"]), None)
    assert (
        secure_tool is not None
    ), f"Tool with '{secure_tool_name}' not found in tools: {[t['name'] for t in tools]}"
    ...
    call_response = app.post_json(
        ...
        {  # type: ignore
            ...
            "params": {
                ...
                "arguments": {"data": "test_data", "auth_token": "bearer_token_123"},
            },
        },
    )
...
def test_pyramid_view_with_basic_auth_integration():
    ...
    @view_config(
        route_name="secure_ftp_endpoint", renderer="json", mcp_security="basic"
    )
    def secure_ftp_view(
        pyramid_request, path: str, username: str, password: str
    ) -> dict:
    ...
    # Initialize MCP
    init_response = app.post_json(
        "/mcp", {"jsonrpc": "2.0", "method": "initialize", "id": 1}  # type: ignore
    )
    assert init_response.status_code == 200
...
def test_pyramid_view_security_parameter_validation():
    ...
    @view_config(
        route_name="validation_endpoint", renderer="json", mcp_security="bearer"
    )
    ...
    call_response = app.post_json(
        ...
        {  # type: ignore
            ...
            "params": {
                ...
                "arguments": {"message": "test", "auth_token": "valid_token"},
            },
        },
    )
    assert call_response.status_code == 200
    ...
    error_response2 = app.post_json(
        ...
        {  # type: ignore
            ...
            "params": {
                "name": validation_tool["name"],
                "arguments": {"message": "test", "auth_token": ""},  # Empty token
            },
        },
    )
...
def test_pyramid_view_without_security_still_works():
    ...
    call_response = app.post_json(
        ...
        {  # type: ignore
            ...
            "method": "tools/call",
            "id": 3,
            "params": {"name": normal_tool["name"], "arguments": {"data": "test_data"}},
        },
    )
    assert call_response.status_code == 200
...
def test_mixed_secure_and_normal_tools():
    ...
    @view_config(route_name="secure_endpoint", renderer="json", mcp_security="bearer")
    ...
    secure_response = app.post_json(
        ...
        {  # type: ignore
            ...
            "params": {
                ...
                "arguments": {"data": "secure_data", "auth_token": "test_token"},
            },
        },
    )
