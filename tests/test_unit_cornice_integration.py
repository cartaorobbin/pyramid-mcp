...
def test_extract_marshmallow_schema_info():
    ...
    class UserSchema(marshmallow.Schema):
        name = fields.String(required=True, metadata={"description": "User's full name"})
        email = fields.Email(required=True, metadata={"description": "User's email address"})
...
def test_add_validation_constraints():
    ...
    field_with_choices = fields.String(validate=validate.OneOf(["draft", "published", "archived"]))
    ...
    field_with_multiple = fields.String(validate=[
...
def test_cornice_service_with_marshmallow_schema():
    ...
    # Create a test schema
    class CreateUserSchema(marshmallow.Schema):
        ...
        age = fields.Integer(validate=marshmallow.validate.Range(min=18, max=120))
    
    # Create Cornice service with schema
    user_service = Service(name="users", path="/users", description="User management")
