...
def test_complete_plugin_integration_scenario(mcp_settings_factory):
    ...
    for tool_func in [scenario_add, scenario_greet]:
        tool_name = tool_func._mcp_tool_config["name"] if hasattr(tool_func, "_mcp_tool_config") else tool_func.__name__
    ...
    config.commit()
    routes = config.get_routes_mapper().get_routes()
    mcp_routes = [route for route in routes if route.name and route.name.startswith("mcp_")]
    assert len(mcp_routes) >= 1
...
def test_plugin_with_fixture_integration(pyramid_mcp_configured):
    ...
    request = {
        "jsonrpc": "2.0",
        "method": "tools/call",
        "params": {"name": "fixture_test_tool", "arguments": {"input_text": "test input"}},
        "id": 1
    }
