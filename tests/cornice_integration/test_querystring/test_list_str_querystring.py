...
class StrsQuerySchema(Schema):
    """Schema for pagination query parameters."""

    strs = fields.List(
        fields.Str,
        required=False,
        metadata={"description": "List of string values"},
    )

    @pre_load
    def pre_load(self, data, **kwargs):
        if "strs" in data and data["strs"]:
            data["strs"] = json.loads(data["strs"])
...
def pagination_service():
    ...
    @users_service.get(
        schema=StrsQuerySchema, validators=(marshmallow_querystring_validator,)
    )
...
def test_cornice_querystring_validation(
    ...
    mcp_request = {
        ...
        "params": {
            "name": "list_list_ints_query_string",
            "arguments": {"querystring": {"strs": ["2", "3"]}},
        },
    }
