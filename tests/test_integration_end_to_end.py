...
def test_auto_discovered_tools_call_real_views():
    ...
    if len(auto_tools) > 0:
        ...
        user_tools = [name for name in tool_names if 'user' in name.lower() and 'update' not in name.lower()]
        if user_tools:
            mcp_call_user = {
                "jsonrpc": "2.0",
                "id": 5,
...
def test_comparison_simulation_vs_real():
    ...
    endpoint_tools = [name for name in tool_names if 'test' in name.lower() or 'endpoint' in name.lower()]
...
def test_complete_pyramid_mcp_workflow(pyramid_mcp_configured, testapp_with_mcp):
    ...
    available_tools = [name for name in tool_names if name in ["workflow_calculator", "text_processor", "get_user_count", "calculate"]]
    assert len(available_tools) > 0, f"Expected some tools to be available, got: {tool_names}"
...
def test_multi_step_integration_scenario():
    ...
    def data_view(request):
        ...
        if data_type == 'numbers':
            ...
            return {"data": [chr(i) for i in range(ord('a'), ord('a') + limit)], "type": "letters"}
        else:
    ...
    init_response = app.post_json('/api/mcp', {
        "jsonrpc": "2.0", "method": "initialize", "id": 1
    })
    assert init_response.status_code == 200
    
    # Step 2: Get available tools
    list_response = app.post_json('/api/mcp', {
    ...
    analyze_response = app.post_json('/api/mcp', {
        "jsonrpc": "2.0",
        "method": "tools/call",
        "params": {
            "name": "data_analyzer",
    ...
    process_response = app.post_json('/api/process', {
        "operation": "sum",
        "values": [1, 2, 3, 4, 5]
    })
    ...
    api_tools = [name for name in tool_names if 'api' in name.lower() or 'data' in name.lower() or 'process' in name.lower()]
    
    if api_tools:
        # Test one of the auto-discovered tools
        auto_tool_response = app.post_json('/api/mcp', {
            "jsonrpc": "2.0",
            "method": "tools/call",
...
def test_error_handling_across_components(testapp_with_mcp):
    ...
    invalid_method_response = testapp_with_mcp.post_json("/mcp", {
        "jsonrpc": "2.0",
    ...
    missing_params_response = testapp_with_mcp.post_json("/mcp", {
        "jsonrpc": "2.0",
        "method": "tools/call",
        "id": 2
    })
    ...
    nonexistent_tool_response = testapp_with_mcp.post_json("/mcp", {
        "jsonrpc": "2.0",
        "method": "tools/call",
