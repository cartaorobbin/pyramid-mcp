...
def test_mcp_calls_protected_route_with_jwt_succeeds(testapp_with_jwt_auth, valid_jwt_token):
    """
...
def test_mcp_calls_protected_route_without_jwt_fails(testapp_with_jwt_auth):
    ...
    assert 'authentication' in result['error']['message'].lower() or 'token' in result['error']['message'].lower()
...
def test_mcp_tool_reflects_pyramid_view_permission(testapp_with_jwt_auth, valid_jwt_token):
    """
    ...
    assert 'authentication' in protected_tool['description'].lower() or 'requires' in protected_tool['description'].lower()


def test_tool_decorator_with_permission_parameter(testapp_with_jwt_auth, valid_jwt_token):
    ...
    headers = {"Authorization": f"Bearer {valid_jwt_token}"}
    response = testapp_with_jwt_auth.post_json('/mcp', protected_request, headers=headers, status=200)
